...
public:
	...
	MDFN_Surface(void* const p_pixels, const uint32 p_width, const uint32 p_height, const uint32 p_pitchinpix)
	{
		...
		{
			pix = (uint32*)calloc(1, p_pitchinpix * p_height * 4);
	...
	~MDFN_Surface()
	{
		if (!pixels_is_external) free(pix);
	}
...
class Deinterlacer
{
 ...
 void Process(MDFN_Surface *surface, MDFN_Rect &DisplayRect, const bool field);
